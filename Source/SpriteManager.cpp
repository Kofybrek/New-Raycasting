...
SpriteManager::SpriteManager() {
  ...
  for (const std::filesystem::directory_entry file :
       std::filesystem::directory_iterator("Resources/Images/")) {
  ...
  // Adding new elements to the vector changes its memory address so we're
  // resizing it then changing the existing ones.
  for (const std::filesystem::directory_entry file :
       std::filesystem::directory_iterator("Resources/Images/")) {
    if (".sprites" == file.path().extension()) {
...
void SpriteManager::draw_sprite(
    const unsigned short i_frame, const std::string &i_sprite_name,
    const sf::Vector2<short> &i_position, sf::RenderWindow &i_window,
    const bool i_mirror_horizontal, const bool i_mirror_vertical,
    const float i_scale_x, const float i_scale_y, const sf::Color &i_color,
  ...
  texture_box.height =
      std::min<unsigned short>(i_texture_box.height, texture_box.height);
  texture_box.width =
      std::min<unsigned short>(i_texture_box.width, texture_box.width);
  ...
  // After some debugging I found out that this is SUPER inefficient. I'm
  // planning to completely rewrite the SpriteManager because of it.
  sprites_data.at(i_sprite_name).sprite->setColor(i_color);
  sprites_data.at(i_sprite_name)
      .sprite->setOrigin(sprites_data.at(i_sprite_name).origin.x,
                         sprites_data.at(i_sprite_name).origin.y);
...
std::string strip(const std::string& str) {
    ...
    // Create a substring containing the stripped characters
    return (start < end) ? std::string(start, end) : std::string();
}

void SpriteManager::load_texture(const unsigned short i_texture_id,
  ...
  textures[i_texture_id].loadFromFile("Resources/Images/" + i_file_name +
                                      ".png");
  ...
  while (std::getline(data_file, line)) {
    ...
    if (0 == line.size()) {
      ...
      case 0: {
        ...
        sprites_data.at(name).image_location =
            "Resources/Images/" + i_file_name + ".png";
...
gbl::SpriteData &
SpriteManager::get_sprite_data(const std::string &i_sprite_name) {
